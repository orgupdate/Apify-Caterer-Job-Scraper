# 🚀 Caterer.com Jobs Scraper – Scrape Caterer.com Jobs to JSON, CSV & Excel

Scrape and aggregate job listings from **Caterer.com** (caterer.com) search results – job title, company, location, salary, posting date and a direct apply link – and export them to JSON, CSV, Excel or an API in a single run.
...
- **🔍 Full-Text Job Search** – Search Caterer.com by keyword, job title or skill and walk every result page automatically.
...
[
  {
    ...
    "posted_via": "Caterer.com",
    "salary": "$120,000 - $150,000 a year",
    "date": "2025-03-25",
    "job_type": "Full-time",
    "URL": "https://caterer.com/..."
  }
]
```
...
Not every Caterer.com listing includes every field – `salary`, `job_type` and exact `date` vary by posting – so handle optional fields gracefully in your integration.
...
run = client.actor("orgupdate/caterer-jobs-scraper").call(run_input=run_input)
...
const run = await client.actor('orgupdate/caterer-jobs-scraper').call(input);
...
- **Find remote Caterer.com jobs via MCP** – let an AI agent pull remote listings with company, source and direct apply links.
- **Track Compass Group's open roles** – monitor one employer's postings on Caterer.com on a daily schedule.
- **Jobs posted in the last 3 days** – recent Caterer.com listings filtered by `datePosted`, feeding a database.
- **Salary benchmark by city** – aggregate Caterer.com pay ranges for one job title across multiple locations.

## 💡 Use Cases

- **Job boards & aggregators** – Populate a niche board with fresh Caterer.com listings, no manual entry.
